"""
Predicts TOA and gate boundaries.
...
def calculate_polyco_phase(MJDObs, rphase_values, rotation_freq_value, mjd_values, polyco_coefficients, nspan, ncoeffs):
    """
    Calculates the phase of the pulsar at a given time (MJDObs).
...
def load_profile_data(file_path, mjd_start):
    """
    Loads observed pulsar profile from a file and extracts time and phase at the peak amplitude point.
...
def estimated_number_of_rotations(MJD_observation_mjd2,observed_toa_mjd1,ref_frequency):
    """
    Estimates the number of rotations of the pulsar.
...
def mjd_to_ist_astropy(mjd_utc):
    """
    Converts MJD to IST using Astropy.
...
# Declare all the relevant time variables 
    ...
    print(f"The predicted pulsar period at {observation_date_mjd2} is {1/rot_freq_mjd2} sec")
    ...
    print(f'gate_width from TOA:{gate_width_ms} in ms ')
